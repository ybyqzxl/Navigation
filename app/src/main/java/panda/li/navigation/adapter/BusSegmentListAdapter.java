...
import panda.li.navigation.route.SchemeBusStep;
			...
			if (item.isWalk() && item.getWalk() != null) {
				...
				holder.parent.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						LatLonPoint startlat=item.getWalk().getOrigin();
						LatLonPoint endlat=item.getWalk().getDestination();
						Intent it=new Intent();
						it.setClass(mContext, BasicWalkNaviActivityNavi.class);
						it.putExtra("startlat",startlat);
						it.putExtra("endlat",endlat);
